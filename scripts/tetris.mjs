    ...
    parse(codestr) {
        ...
        for (let code of codestr.split(/\n/g)) {
            ...
            if (code.match(/^-?\d+:/)) {
                ...
                this.assert(!Number.isNaN(parsed), lineNumber, `ラベルの数値をパース出来ません`);
...
if (process.argv.length < 3) {
    console.log("Usage: tetris.mjs <sourcefile>");
    process.exit(1);
}
...
const tetriscore = new TetrisInterpreter(code, input);

if (!tetriscore.errorMessage) {
  for (let i = 0; i < 10000 && !tetriscore.finished; i++) {
    const res = tetriscore.execute1Step();
    if (res != '') {
